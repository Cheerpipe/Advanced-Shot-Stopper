...
from custom_components.open_brew_by_weight.diagnostics import (
...
from custom_components.open_brew_by_weight.repairs import async_create_webhook_repair
...
def _coordinator(hass):
    entry = config_entry()
    api = api_mock()
    coordinator = OpenBrewByWeightCoordinator(hass, entry, entry.data["device_id"], api)
...
async def test_webhooks_reject_other_boots_until_reconciled(hass) -> None:
    ...
    schedule.assert_called_once_with(
        "open_brew_by_weight newer-boot reconciliation", required_boot_id=125
    )
...
async def test_successful_boot_refresh_cancels_sleeping_recovery(hass) -> None:
    ...
    with patch(
        "custom_components.open_brew_by_weight.coordinator.asyncio.sleep",
        side_effect=sleep,
    ):
...
async def test_runtime_register_validate_and_transaction(hass) -> None:
    ...
    coordinator, api, _entry = _coordinator(hass)
    runtime = OpenBrewByWeightRuntimeData(
        hass, api, coordinator, WEBHOOK_ID, "http://home/webhook"
    )
    with patch(
        "custom_components.open_brew_by_weight.runtime.webhook.async_register"
    ) as register:
    ...
    with (
        patch(
            "custom_components.open_brew_by_weight.runtime.secrets.token_urlsafe",
            return_value="proof",
        ),
    ...
    with patch(
        "custom_components.open_brew_by_weight.runtime.webhook.async_unregister"
    ) as unregister:

...
from custom_components.open_brew_by_weight.models import ProtocolError
...
@pytest.mark.parametrize("host", ["192.168.1.8", "controller.local", "controller.local:8080"])
def test_normalize_host(host: str) -> None:
    assert normalize_host(host) == host


@pytest.mark.parametrize(
    "host", ["", "http://controller", "user@controller", "controller/path", "controller:70000"]
)
...
async def test_reads_snapshot_and_presets() -> None:
    ...
    api = OpenBrewByWeightApi(session, "controller.local")
...
async def test_transport_and_response_validation() -> None:
    ...
    with pytest.raises(CannotConnect):
        await OpenBrewByWeightApi(BrokenSession(), "controller").async_snapshot()

    response = Response(200, {})
    response.json.side_effect = ValueError
    with pytest.raises(ProtocolError, match="not JSON"):
        await OpenBrewByWeightApi._decode(response)
    with pytest.raises(ProtocolError, match="object"):
        await OpenBrewByWeightApi._decode(Response(200, []))
    with pytest.raises(RequestRejected) as raised:
        await OpenBrewByWeightApi._decode(Response(500, {}))
...
async def test_request_polling_states(monkeypatch) -> None:
    ...
    await OpenBrewByWeightApi(session, "controller").async_wait_request(7)
    ...
    for payload, code in (
        ...
        with pytest.raises(RequestRejected) as raised:
            await OpenBrewByWeightApi(Session(Response(200, payload)), "controller").async_wait_request(7)
...
async def test_all_mutation_shapes() -> None:
    ...
    api = OpenBrewByWeightApi(session, "controller")
    ...
    assert session.calls[0][0][:2] == (
        "PUT",
        "http://controller/api/v1/integration/webhook",
    )
    ...
    with pytest.raises(ProtocolError, match="requestId"):
        await OpenBrewByWeightApi(
            Session(Response(202, {})), "controller"
        ).async_select_preset(2)
    with pytest.raises(ProtocolError, match="restart"):
        await OpenBrewByWeightApi(
            Session(Response(200, {"requestId": True})), "controller"
        ).async_restart()

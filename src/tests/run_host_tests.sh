...
ota_state_concurrency_binary=${TMPDIR:-/tmp}/shot_stopper_ota_state_concurrency_host_test
ota_state_concurrency_tsan=${TMPDIR:-/tmp}/shot_stopper_ota_state_concurrency_host_test_tsan
...
resource_owner_binary=${TMPDIR:-/tmp}/shot_stopper_resource_owner_host_test
resource_owner_sanitized=${TMPDIR:-/tmp}/shot_stopper_resource_owner_host_test_sanitized
...
"$cxx" -std=c++17 -Wall -Wextra -Werror -pedantic -pthread \
  "$test_dir/ota_state_concurrency_host_test.cpp" \
  -o "$ota_state_concurrency_binary"
"$ota_state_concurrency_binary"

"$cxx" -std=c++17 -O1 -g -Wall -Wextra -Werror -pedantic \
  -pthread -fno-omit-frame-pointer -fsanitize=thread \
  "$test_dir/ota_state_concurrency_host_test.cpp" \
  -o "$ota_state_concurrency_tsan"
TSAN_OPTIONS=halt_on_error=1 "$ota_state_concurrency_tsan"
...
"$cxx" -std=c++17 -Wall -Wextra -Werror -pedantic \
  "$test_dir/resource_owner_host_test.cpp" \
  -o "$resource_owner_binary"
"$resource_owner_binary"

"$cxx" -std=c++17 -Wall -Wextra -Werror -pedantic \
  -fno-omit-frame-pointer -fsanitize=address,undefined \
  "$test_dir/resource_owner_host_test.cpp" \
  -o "$resource_owner_sanitized"
ASAN_OPTIONS=detect_leaks=0:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1 \
  "$resource_owner_sanitized"
...
python3 "$repo_root/scripts/check_architecture.py"

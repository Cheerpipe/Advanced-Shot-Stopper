...
| **Local discovery (mDNS)** | on with STA HTTP | Publishes `_http._tcp` on port 80 when STA and HTTP are ready and SoftAP is down. With `MIN_MODEM`, every 30 s it wakes Wi-Fi for 500 ms, announces over IPv4, keeps Wi-Fi awake for another 500 ms, then restores the current power policy. Pauses during active shots and scale connection attempts, then resumes automatically. Initialization failures retry at most once per 5 seconds. |

...
| **Local discovery (mDNS)** | on with STA HTTP | Publishes `_http._tcp` on port 80 when STA and HTTP are ready and SoftAP is down. Every 30 s it refreshes the complete HTTP service; with `MIN_MODEM`, it first wakes Wi-Fi for 500 ms, keeps it awake for another 500 ms after announcing, then restores the current power policy. Pauses during active shots and scale connection attempts, then resumes automatically. Initialization failures retry at most once per 5 seconds. |

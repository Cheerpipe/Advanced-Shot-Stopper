...
For firmware built with `OPEN_BREW_BY_WEIGHT_MACHINE_TYPE=1` (button only) or `2`
...
| **Assume idle when the scale connects** | ON | ON / OFF | Switch-only (`OPEN_BREW_BY_WEIGHT_MACHINE_TYPE=1`). When the scale connects, treat the group as idle (Confirmed off). Does not pulse the relay. Skipped while a brew cycle is active. |
...
- **Button-only, no scale:** the machine may start from a tap, but the brew orchestrator

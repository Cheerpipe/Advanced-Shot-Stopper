...
| scale NimBLE peer procedure | EspressoScaleBLE client, called only by ScaleService | owns final admission for scanning, connection, discovery, commands, RSSI, and teardown; its GAP callback starts the fixed 3,000 ms disconnect barrier before notifying firmware, and no caller can shorten or bypass it |

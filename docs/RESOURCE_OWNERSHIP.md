...
| scale NimBLE peer procedure | EspressoScaleBLE client, called only by ScaleService | owns final admission for scanning, connection, discovery, commands, RSSI, and teardown; its GAP callback immediately rejects new procedures, then starts the full 3,000 ms quiet interval when any already-admitted submission returns; no caller can shorten or bypass it |

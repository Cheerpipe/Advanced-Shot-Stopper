...
| shared NimBLE peer procedure | `ShotStopperBleArbiter`; ScaleService owns scanning and has priority over the one build-selected machine integration | fixed observers and one scan coordinator seal before host start; the coordinator pauses discovery before an admitted machine procedure and keeps it paused for the lease without dropping a linked scale; a scale candidate or critical interval prevents admission or invalidates the machine lease, whose client must quarantine late callbacks, abort and release it; the selected adapter adds no task or BLE-host owner |

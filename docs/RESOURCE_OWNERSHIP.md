...
| shared NimBLE peer procedure | `ShotStopperBleArbiter`; ScaleService has priority over the one build-selected machine integration | fixed observer slots seal before host start; a scale candidate or critical interval invalidates the machine lease, whose client must abort and release it; the selected machine adapter adds no task or BLE-host owner |

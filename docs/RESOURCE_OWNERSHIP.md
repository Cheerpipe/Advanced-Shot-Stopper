...
| shared NimBLE peer procedure | `ShotStopperBleArbiter`; ScaleService owns scanning and has priority over the one build-selected machine integration | fixed observers and one scan coordinator seal before host start; the coordinator supplies bounded observation windows and pauses discovery before an admitted machine procedure without dropping a linked scale; a scale candidate or critical interval invalidates the machine lease, whose client must abort and release it; the selected adapter adds no task or BLE-host owner |

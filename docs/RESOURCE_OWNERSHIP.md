...
| Micra cloud HTTPS client | build-selected `ShotStopperMicraService` worker | boot-lifetime 8 KiB task and bounded PSRAM workspace; each request closes its socket, STA/AP/shot transitions cancel active I/O, and Disconnect wipes credentials/session buffers and destroys the client handle |
